...
// TODO: may not need this as we can directly update to final state when hook execution finishes,
// we do not need to monitor the intermediate states (e.g. running) in the database, as the agent
// will directly update to Completed/Failed/Cancelled state once the hook execution finishes or
// fails. We can still keep track of the Running state in memory if needed, but it might not be
// necessary to persist it in the database.
