...
impl Executor {
    ...
    pub async fn execute(
        ...
        self.announce_ex(TaskExecState::FetchResource, 360).await;
        let timeout_until = tokio::time::Instant::now() + RESOURCE_FETCH_BUDGET;
        for resource in spec.resources {
            match download_resource(self, resource, timeout_until).await {
                Ok(()) => {}
                ...
                Err(ResourceError::Other(e)) => {
                    self.announce_ex(TaskExecState::FetchResourceError, 60)
                        .await;
                    return Err(e);
                }
                Err(kind) => {
                    ...
                    self.report_abandoned(msg).await?;
                    self.announce_ex(state, 60).await;
                    self.announce_ex(TaskExecState::TaskCommitted, 60).await;
                    return Ok(());
                }
            }
        }
        ...
        if let Some((watched_uuid, watched_state)) = exec_options.and_then(|opts| opts.watch) {
            ...
            if self.client.can_watch() {
                self.announce(TaskExecState::Watch).await;
                let cancel_token = self.cancel_token.clone();
                tokio::select! {
                    biased;
                    _ = cancel_token.cancelled() => {
                        tracing::info!("Watching interrupted by shutdown signal");
                        self.announce_ex(TaskExecState::WorkerExited, 60).await;
                        return Ok(());
                    },
                    _ = self.client.watch(&watched_uuid, watched_state) => {},
                    _ = tokio::time::sleep_until(timeout_until) => {
                        tracing::debug!("Watching timeout, commit this task as cancelled");
                        self.report_abandoned(TaskResultMessage::WatchTimeout).await?;
                        self.announce_ex(TaskExecState::WatchTimeout, 60).await;
                        self.announce_ex(TaskExecState::TaskCommitted, 60).await;
                        return Ok(());
                    }
                }
        ...
        match exec_timeout {
            ...
            Some(timeout) => {
                self.announce_ex(TaskExecState::ExecPending, timeout.as_secs() + 60)
                    .await
            }
            // Nothing to expire against: the state stands until the next announce.
            None => self.announce(TaskExecState::ExecPending).await,
        }

        // Setup the child-task hand-off file and clean up any stale one
        let new_task_path = self.cache_path.join("new_task.json");
        ...
        command
            .args(spec.args)
            .envs(spec.envs)
            .envs(self.client.exec_env())
            .env("MITO_RESULT_DIR", self.cache_path.join("result"))
            .env("MITO_EXEC_DIR", self.cache_path.join("exec"))
            .env("MITO_RESOURCE_DIR", self.cache_path.join("resource"))
        ...
        if self.client.supports_child_tasks() {
        ...
        let mut child = command.spawn().inspect_err(|e| {
            tracing::error!("Failed to spawn {}: {}", self.client.describe(), e);
        })?;
        self.announce(TaskExecState::ExecSpawned).await;
        ...
        let output = tokio::select! {
            ...
            output = process_output => {
                self.announce_ex(TaskExecState::ExecFinished, 660).await;
                output.map(ExecResult::Finish)
            },
            _ = exec_deadline => {
                tracing::debug!("Execution timeout");
                self.announce_ex(TaskExecState::ExecTimeout, 60).await;
                if let Some(id) = child.id() {
        ...
        tracing::debug!("Execution of {} finished", self.client.describe());
        self.announce_ex(TaskExecState::UploadResult, 660).await;
        self.process_exec_result(output).await?;
        Ok(())
    }

    /// Archive whatever the process produced, upload it, then commit the outcome.
    async fn process_exec_result(&mut self, output: ExecResult) -> crate::error::Result<()> {
        ...
        let archive_cancel_token = self.cancel_token.clone();
        let archive_cache_path = self.cache_path.clone();
        ...
        let upload_artifact_fut = async {
            while let Some((content_type, content_length)) = rx.recv().await {
                ...
                loop {
                    let file =
                        tokio::fs::File::open(self.cache_path.join(content_type.to_string()))
                            .await?;
        ...
        tokio::select! {
            ...
            _ = tokio::time::sleep_until(timeout_until) => {
                ...
                self
                    ...
                    .await?;
                self.announce_ex(TaskExecState::UploadResultTimeout, 60).await;
                self.announce_ex(TaskExecState::TaskCommitted, 60).await;
                archive_hd.await??;
            }
            res = upload_artifact_fut => {
                ...
                self.announce_ex(TaskExecState::UploadResultFinished, 60).await;

                let child_task_submitted = self.submit_child_task_if_present().await;
        ...
        reset_workspace(&self.cache_path).await?;
        Ok(())
    }
    ...
    async fn submit_child_task_if_present(&mut self) -> bool {
        if !self.client.supports_child_tasks() {
            return true;
        }
        let new_task_path = self.cache_path.join("new_task.json");
        ...
        let submitted = self.client.submit_child_task(submit_req).await;
