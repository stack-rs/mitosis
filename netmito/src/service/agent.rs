...
pub async fn user_register_agent(
    ...
    let metadata_json: Option<serde_json::Value> = req
        ...
        .transpose()
        .map_err(|e| Error::Custom(format!("Invalid metadata: {e}")))?;

    let lifetime = req
        .lifetime
        .unwrap_or(std::time::Duration::from_secs(DEFAULT_AGENT_LIFETIME_SECS));
...
pub async fn agent_heartbeat(
    ...
    if req.state == AgentState::Idle
        && db_agent.assigned_task_suite_id.is_none()
        && check_suite_available_for_agent(agent_id, pool).await?
    {
        let _ = AgentWsRouter::notify(
            &pool.ws_router_tx,
            agent_uuid,
...
pub async fn user_query_agents(
    ...
    if query.count {
        ...
        stmt.columns([
            ...
            (Agent::Entity, Agent::Column::Metadata),
...
pub async fn agent_accept_suite(
    ...
    let schedule: WorkerSchedulePlan = serde_json::from_value(suite.worker_schedule.clone().take())
        .unwrap_or(WorkerSchedulePlan::FixedWorkers {
            worker_count: 1,
            cpu_binding: None,
    ...
    let _ = pool
        ...
        .send(SuiteDispatcherOp::UpdateCapacity {
            suite_id: suite.id,
            batch_size,
        });
...
async fn remove_agent(agent: Agent::Model, pool: &InfraPool) -> Result<()> {
    ...
    Agent::Entity::delete_by_id(agent.id).exec(&pool.db).await?;
