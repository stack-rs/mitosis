...
impl AgentHeartbeatQueue {
    ...
    async fn handle_timeout(&mut self) -> crate::error::Result<()> {
        if let Some(true) = self.agents.peek().map(|(_, r)| r.0 <= Instant::now()) {
            ...
            // Fetch the agent to get its UUID (needed for task reclamation by runner_uuid)
            ...
            match mark_result {
                Ok(Ok(_)) => {}
                Ok(Err(e)) => {
                    tracing::error!(agent_id = agent_id, "Failed to mark agent offline: {}", e);
                    return Err(e.into());
                }
                Err(_) => {
