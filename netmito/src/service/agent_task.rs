...
//! 3. For each ID: atomically `SET state=Running, runner_uuid=agent_uuid WHERE id=? AND state=Ready`.
...
//! ## Suite completion
//!
//! On `Commit`, if the task belongs to a suite, `incomplete_tasks` is decremented.
        ...
        match tasks {
            Ok(tasks) => {
                let pairs: Vec<(i64, i32)> =
                    tasks.into_iter().map(|t| (t.id, t.priority)).collect();
                let _ = pool
...
pub async fn agent_fetch_tasks(
        ...
        .is_err()
    {
        return Err(Error::Custom(
            "suite task dispatcher channel closed".to_string(),
        ));
    }
    ...
    for task_id in fetch_result.task_ids {
        let result = ActiveTasks::Entity::update_many()
            ...
            .filter(ActiveTasks::Column::State.eq(TaskState::Ready))
            .filter(ActiveTasks::Column::RunnerUuid.is_null())
            .exec(&pool.db)
            .await?;
...
/// Decrement `incomplete_tasks` for a suite after a task is committed.
/// Transitions the suite to `Complete` if `incomplete_tasks` reaches 0.
async fn commit_suite_task(
    pool: &InfraPool,
    suite_id: i64,
    now: TimeDateTimeWithTimeZone,
) -> Result<()> {
    // Decrement incomplete_tasks (guard against going below 0).
    ...
    // If incomplete_tasks reached 0 and the suite is Open or Closed → mark Complete.
