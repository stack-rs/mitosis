...
/// Decrements incomplete_tasks when a task completes or is cancelled.
/// If incomplete_tasks reaches 0, transitions the suite to Complete state.
///
    ...
    C: TransactionTrait,
{
    db.transaction::<_, (), Error>(|txn| {
        Box::pin(async move {
            // First, decrement the incomplete_tasks counter
            // We only decrement if incomplete_tasks > 0 to prevent negative values
            let updated = TaskSuites::Entity::update_many()
                .col_expr(
            ...
            if let Some(suite) = updated.into_iter().next() {
                if suite.incomplete_tasks == 0 && !suite.state.is_terminal() {
...
async fn get_assigned_agent_uuids(
    ...
    let stmt = Query::select()
        ...
        .and_where(
            Expr::col((TaskSuiteAgent::Entity, TaskSuiteAgent::Column::TaskSuiteId)).eq(suite_id),
        )
        .to_owned();
...
pub async fn user_cancel_task_suite(
    ...
    // after commit. running_task_infos carries (runner_uuid, task_uuid) pairs for any
    ...
    if !running_task_infos.is_empty() {
        ...
        for (runner_uuid, task_uuid) in running_task_infos {
            if let Some(agent_uuid) = runner_uuid {
...
pub async fn user_refresh_suite_agents(
    ...
    if !resp.added_agents.is_empty() {
        if let Ok(Some(suite)) = TaskSuites::Entity::find()
            .filter(TaskSuites::Column::Uuid.eq(suite_uuid))
            .one(&pool.db)
            .await
        {
