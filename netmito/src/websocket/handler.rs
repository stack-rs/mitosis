...
/// **Piece 3.4: Handle heartbeat message - updates manager state and last_heartbeat timestamp**
async fn handle_heartbeat(
    ...
    // Update manager state and last_heartbeat using SeaORM
    use crate::entity::node_managers;

    let now = time::OffsetDateTime::now_utc();

    // Use update_many to update the manager record
...
/// **Piece 3.5: Handle fetch task request - returns a task from the manager's assigned suite**
async fn handle_fetch_task(
    ...
    // Get manager's assigned suite using SeaORM
    use crate::entity::node_managers;
    ...
    let task = if let Some(manager) = manager {
        if let Some(_suite_id) = manager.assigned_task_suite_id {
            // TODO: Fetch task from assigned suite
            // This will be implemented when the task fetching logic for suites is ready
            // For now, return None to indicate no tasks available
            tracing::debug!(
                manager_uuid = %manager_uuid,
                "Manager has assigned suite, but task fetching not yet implemented"
            );
            None
        } else {
            tracing::debug!(
                manager_uuid = %manager_uuid,
                "Manager has no assigned suite"
            );
            None
        }
    } else {
    ...
    // Send response back using the connection registry
