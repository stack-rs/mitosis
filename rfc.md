...
pub struct TaskSuite {
    ...
    pub labels: HashMap<String, String>,   // For querying
    ...
    pub auto_close_timeout: Option<Duration>, // Inactivity timeout
...
```
  ...
  Open → Closed (auto_close_timeout elapsed, no new tasks)
...
CREATE TABLE task_suites (
    ...
    labels JSONB NOT NULL DEFAULT '{}',  -- key-value pairs for querying
    ...
    auto_close_timeout BIGINT,  -- Timeout in seconds (NULL = never auto-close)
...
-- Index for label-based queries (similar to existing task queries)
-- Supports queries like: labels @> '{"project": "ml-training"}'::jsonb
```
...
#### Auto-Update Suite Pending Count

```sql
CREATE OR REPLACE FUNCTION update_suite_pending_count()
RETURNS TRIGGER AS $$
BEGIN
    ...
    ELSIF TG_OP = 'UPDATE' AND NEW.task_suite_id IS NOT NULL THEN
        IF OLD.state != NEW.state AND NEW.state IN (3, 4) THEN
            ...
            UPDATE task_suites
            SET pending_task_count = GREATEST(pending_task_count - 1, 0),
                updated_at = NOW()
            WHERE id = NEW.task_suite_id;
        END IF;
...
FOR EACH ROW
EXECUTE FUNCTION update_suite_pending_count();
```
...
RETURNS void AS $$
BEGIN
    -- Transition OPEN → CLOSED (auto_close_timeout elapsed)
    UPDATE task_suites
    SET state = 1,  -- Closed
        updated_at = NOW()
    WHERE state = 0  -- Open
      ...
      AND EXTRACT(EPOCH FROM (NOW() - last_task_submitted_at)) > auto_close_timeout
...
|------------|-------|----------|--------|
| (None) | First task submitted | Open | Set `last_task_submitted_at`, increment `pending_task_count` |
| Open | `auto_close_timeout` elapsed | Closed | Background job detects timeout |
...
| **Auto-close Timeout** | Inactivity timeout after which suite transitions from Open to Closed |
